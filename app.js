...
(() => {
  ...
  style.textContent = ".tefas-open{border:0;border-radius:999px;padding:8px 13px;background:var(--card);color:var(--muted);font:600 13px -apple-system,BlinkMacSystemFont,Segoe UI,sans-serif;box-shadow:0 1px 3px #0001;text-decoration:none;white-space:nowrap}.date-range{display:flex;align-items:end;gap:7px;flex-wrap:wrap;margin-left:auto}.date-range label{display:grid;gap:3px;color:var(--muted);font-size:11px;font-weight:600}.date-range input{border:1px solid var(--line);border-radius:10px;padding:7px;background:var(--card);color:var(--text);font:inherit}@media(max-width:520px){.date-range{width:100%;margin-left:0;display:grid;grid-template-columns:minmax(0,1fr) minmax(0,1fr);gap:6px}.date-range label{min-width:0}.date-range input{min-width:0;width:100%;padding:6px;font-size:13px}.date-range button{grid-column:1/-1;justify-self:end;padding:7px 10px}}";
